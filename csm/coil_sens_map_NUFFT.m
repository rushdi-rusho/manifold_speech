function [coilimages] = coil_sens_map_NUFFT(kdata,trajectory,w,N,useGPU)
...
if(useGPU)
    ...
    FT= NUFFT(trajectory(:),ones(nReadouts*nInterleaves*nFrames,1),0,0,[N,N]);
...
for i=1:nCh
    temp = FT'*(kdata(:,i).*sqrt(w(:)));
    coilimages(:,:,i)= reshape(pcg(ATA,temp(:),1e-6,10),[N,N]);
end
